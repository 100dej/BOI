Public Class Frm205AllocateFOHDirectNull
    ...
    Private Sub cmdExec_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim dt As DataTable = cx.GetdataTable(String.Format("select * from boi.dbo.FT_GroupCost('{0}','{1}') where mcctrboino = ''  and mcctrtype_allocate not in ('Profiles Service','FITTING Service','PIPE Service','Service')", xMonth, xYear))
        ...
        Dim dt1 As DataTable = cx.GetdataTable(String.Format("select * from boi.dbo.FT_AllocateCost_NullDirect('{0}','{1}') order by mcostelement, mproduct", xMonth, xYear))
        dv = dt1.DefaultView
    ...
    Private Sub cmdExport1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        cdl.FileName = String.Format("Report Allocate Direct not fix BOI month {0} year {1}", xMonth, xYear)
    ...
    Private Sub cmdExport2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Clear_List()
        Dim cdl As New SaveFileDialog
        cdl.FileName = String.Format("Report Allocate Direct not fix BOI details month {0} year {1}", xMonth, xYear)
        cdl.Filter = "XLS File|*.xls"
        If cdl.ShowDialog = Windows.Forms.DialogResult.OK Then
