Public Class Frm202DetailsFOH
    ...
    Private Sub cmdExec_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim dt As DataTable = cx.GetdataTable(String.Format("select * from boi.dbo.FT_SummaryCostDetails('{0}','{1}') order by mcostelement", xMonth, xYear))
        ...
        Dim dt1 As DataTable = cx.GetdataTable(String.Format("select * from boi.dbo.FT_CostDetails('{0}','{1}')", xMonth, xYear))
        dv = dt1.DefaultView
        dtgDetails.DataSource = dv
        cx.GridToList(dtgDetails)
        Label4.Text = String.Format("{0:N2}", dt1.Compute("sum(mvaluecocurr)", ""))
    ...
    Private Sub cmdExport1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        cdl.FileName = String.Format("Report Summary actual cost month {0} year {1}", xMonth, xYear)
    ...
    Private Sub cmdExport2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Clear_List()
        Dim cdl As New SaveFileDialog
        cdl.FileName = String.Format("Report Actual cost details month {0} year {1}", xMonth, xYear)
        cdl.Filter = "XLS File|*.xls"
        If cdl.ShowDialog = Windows.Forms.DialogResult.OK Then
